...
import { CrearLligaPostController } from "../controllers/controllers-lliga/CrearLligaPostController";
...
export function postCrearLliga(router: Router): void {
    ...
    const controller = new CrearLligaPostController(lligaService, usuariEnLligaService);
    controller.run = controller.run.bind(controller);

    router.post('/api/lliga', new Auth().authenticate, controller.run);
}