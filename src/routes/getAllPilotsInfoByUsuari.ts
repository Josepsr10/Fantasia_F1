...
import { AllPilotsInfoGetController } from "../controllers/controllers-pilot/AllPilotsInfoOfUsuariGetController";

export function getAllPilotsInfoByUsuari(router: Router): void {
    ...
    const controller = new AllPilotsInfoGetController(usuariEnLligaService);

    controller.run = controller.run.bind(controller);

    router.get('/api/usuari/pilots-info', new Auth().authenticate, controller.run);
}